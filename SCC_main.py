...
####################################################################################
# File name : SCC_main.py
# Author: Parisa Mojiri 
# Date created: 16/02/2016 
# Description: The main code to run SCC slump test
...
# 	 - python SCC_main.py --image images/example_01.jpg --platesize 1000
# 	 - python SCC_main.py --video videos/example_01.mp4 --platesize 1000
#
####################################################################################
#
...
ezxf = xlwt.easyxf
hdngs = ['  Time (s)   ', '   edge distance from center point   ',     '    Diameter_ew (mm)   ', '     Diameter_ns (mm)     ']
...
################################################################################
#
# if the video argument is None, then we are reading image file (The image file should be the final image detecting four corners of the plate and the slipped concrete:
if args.get("video", None) is None:
    ...
    persp = Perspective_plate(image, plate_size)
    ...
    warpedImage_cropped = foreground.foreground_detection(warpedImage, (plate_size/2 - 350), (plate_size/2 - 350), (plate_size/2 + 200), (plate_size/2 + 200))

    # concrete detection
    cx = warpedImage.shape[1]/2
    cy = warpedImage.shape[0]/2
    r, detected_area, d_ew, r_ns = radius.radius_detection(warpedImage_cropped, cx, cy, (plate_size/2 - 350) )
	...
	data.append([0, r[i], d_ew, r_ns])
    write_xls(filename_image, "SCC_image", hdngs, data, heading_xf)
...
# otherwise, we are reading from a video:
    ...
    persp = Perspective_plate(first_frame, plate_size)
    warpedImage, transformationMatrix = persp.houghlinePmethod()
...
#   reading each frame of the video
    ...
    while True:
		...
		if t_start is not None:
				...
				############## detection of the bottom of cone  #####################################
    				...
    				obj_mask_1, obj_color_detected_1 = object_detection.Obj_color_detection(warpedImage, [0, 100, 100], [5, 255, 255])
    				obj_mask_2, obj_color_detected_2= object_detection.Obj_color_detection(warpedImage, [170, 100, 100], [180, 255, 255])
    				obj_mask = obj_mask_1 + obj_mask_2 
				cnts_c, hierarchy_c = object_detection.find_obj_contours(obj_mask)
                                yc = []
    	   			if len(cnts_c) > 0:
					for i in cnts_c:
				...
				if w * h >= 20000:
                                	...
                                	################################### radius detection ###########################
                        		r, detected_area, d_ew, r_ns = radius.radius_detection(new_mask_org, cx, cy, cone_bottom_y[0] )
                                	#print r
                                	cv2.imshow('detected', detected_area)
                                	# write the arpedImageframe
					...
					for i in range(0, len(r), 20):
						data.append([time, r[i], d_ew, r_ns]) 
    ...
    write_xls(filename_video, 'SCC_video', hdngs, data, heading_xf)
