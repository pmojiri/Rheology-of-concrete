...
####################################################################################
# File name : SLUMP_test.py
# Author: Parisa Mojiri 
# Date created: 16/02/2016 
# Description: SLUMP test class
...
class SLUMP_Test(object):
	...
	self.hdngs = ['  Time (s)   ', '   edge distance from center point   ', '     Diameter_ew (mm)   ', '     radius_ns (mm)     ', '     Height (mm)   ']
	...
	#and the slipped concrete, complete cone, and the slipped concrete::	
    def slump_image(self):
    	# reading the inputs:
    	image = self.image.read_image(self.filename)
    	plate_size = self.plate_size
	new_image_size = self.imagesize

    	# computing trasformation matrix calling houghlineP method (plate perspective) and warped image (Top-down view of the plate)
    	persp_plate = Perspective_plate(image, plate_size, self.plate_color_min, self.plate_color_max)
    	...
    	warpedImage_cropped = self.foreground.foreground_detection(warpedImage_plate, (plate_size/2 - 350), (plate_size/2 - 350), (plate_size/2 + 200), (plate_size/2 + 200))

    	# concrete detection to extract the radius
    	cx = warpedImage_plate.shape[1]/2
    	cy = warpedImage_plate.shape[0]/2
    	r, detected_area, d_ew, r_ns = self.radius.radius_detection(warpedImage_cropped, cx, cy, cy)
    	...
    	h, height_detected = self.height.height_detection(frame, x_top, x_bottom)
    	#cv2.imshow('height_detected', height_detected)
    	#cv2.waitKey(0)

    	# save the detected area and write the data on a excel file
    	cv2.imwrite("images/result_images/slump_height_detected.jpg", height_detected)
    	for i in xrange(0, len(r)):
		self.data.append([0, r[i], d_ew, r_ns, int(np.abs(300 - h))])
    	self.write_xls(self.filename_image, 'SLUMP_image', self.hdngs, self.data, self.heading_xf)
	...
	return d_ew, int(np.abs(300 - h))


	# otherwise, we are reading from a video:
    def slump_video(self):
    	# reading the inputs:    
    	...
    	cv2.imwrite("images/result_images/SLUMP_first_frame.jpg", first_frame)

    	# computing trasformation matrix calling houghlineP method (plate perspective) and warped image (Top-down view of the plate)
    	persp_plate = Perspective_plate(first_frame, plate_size, self.plate_color_min, self.plate_color_max)
    	warpedImage_plate, transformationMatrix_plate = persp_plate.houghlinePmethod()
    	...
    	video_out = cv2.VideoWriter("videos/SLUMP_warped_plate.avi", fourcc, 30, (plate_size, plate_size))
    	video_cropped = cv2.VideoWriter("videos/SLUMP_cropped_plate.avi", fourcc, 30, (plate_size, plate_size))
    	video_detected = cv2.VideoWriter("videos/SLUMP_radius_detected.avi", fourcc, 30, (plate_size, plate_size))

    	first_frame = imutils.resize(first_frame, width = 900)
    	video_out_cone = cv2.VideoWriter("videos/SLUMP_warped_cone.avi", fourcc, 30, (new_image_size, new_image_size))
    	video_height_detected = cv2.VideoWriter("videos/SLUMP_height_detected.avi", fourcc, 30, (first_frame.shape[1], first_frame.shape[0]))
    	...
    	while True:
			...
			if t_start is not None:
					...
					warpedImage_plate_copy = warpedImage_plate.copy()
        				...
        				video_out_cone.write(warpedImage_cone)

                        		#self.foreground.foreground_detection(warpedImage_plate, x, y, w, h)
					...
					if w * h >= 20000:
                        			...
                        			r, detected_area, d_ew, r_ns = self.radius.radius_detection(new_mask_org, cx, cy, cone_bottom[1])
                                		#print r
                                		#cv2.imshow('radius_detected', detected_area)
                                		# write the arpedImageframe
        		        		video_detected.write(detected_area)
                                		#self.foreground.foreground_detection(new_mask, x, y, w, h)
						...
						slump_height = int(np.abs(300 - h))
							...
							self.data.append([time, r[i], d_ew, r_ns, slump_height])
	...
	video_detected.release()
    	video_out_cone.release()
	video_height_detected.release()
    	#print data on a excel file
    	self.write_xls(self.filename_video, 'SLUMP_video', self.hdngs, self.data, self.heading_xf)
	return d_ew, slump_height, test_time
